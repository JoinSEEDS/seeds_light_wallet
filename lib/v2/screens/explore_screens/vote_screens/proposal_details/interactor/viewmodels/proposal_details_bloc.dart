...
import 'package:seeds/v2/screens/explore_screens/vote_screens/proposal_details/interactor/mappers/proposal_details_data_state_mapper.dart';
import '../usecases/get_proposal_details_data_use_case.dart';
...
class ProposalDetailsBloc extends Bloc<ProposalDetailsEvent, ProposalDetailsState> {
  ...
  Stream<ProposalDetailsState> mapEventToState(ProposalDetailsEvent event) async* {
    if (event is OnLoadProposalData) {
      ...
      List<Result> results = await GetProposalDetailsDataUseCase().run(state.proposals[state.currentIndex].creator);
      yield ProposalDetailsDataStateMapper().mapResultsToState(state, results);
    }
