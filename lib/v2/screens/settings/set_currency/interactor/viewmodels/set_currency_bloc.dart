...
import 'package:seeds/v2/screens/settings/set_currency/interactor/mappers/rates_state_mapper.dart';
import 'package:seeds/v2/screens/settings/set_currency/interactor/usecases/get_fiat_rates_use_case.dart';
  ...
  Stream<SetCurrencyState> mapEventToState(SetCurrencyEvent event) async* {
    if (event is OnQueryChanged) {
      if (event.query.isNotEmpty && event.query.length == 3 && event.query != state.currentQuery) {
        ...
        yield mapResultToState(state, result);
      } else {
