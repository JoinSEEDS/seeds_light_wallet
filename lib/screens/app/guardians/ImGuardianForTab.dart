...
class _ImGuardianForState extends State<ImGuardianForTab> {
  ...
  Widget build(BuildContext context) {
    return FutureBuilder<QuerySnapshot>(
        future: FirebaseDatabaseService().getImGuardiansFor(SettingsNotifier.of(context).accountName),
        builder: (context, guardSnaps) {
          if (guardSnaps.hasData) {
            Iterable<String> users = guardSnaps.data.docs.map((e) => e.id);
            Iterable<MemberModel> filteredList = widget.members.where((e) => users.contains(e.account));
