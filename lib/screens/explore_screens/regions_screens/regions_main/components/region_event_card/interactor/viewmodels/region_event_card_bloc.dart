...
import 'package:seeds/screens/explore_screens/regions_screens/regions_main/components/region_event_card/interactor/usecases/get_n_profiles_use_case.dart';
...
class RegionEventCardBloc extends Bloc<RegionEventCardEvent, RegionEventCardState> {
  ...
  Future<void> _onLoadRegionEventMembers(OnLoadRegionEventMembers event, Emitter<RegionEventCardState> emit) async {
    if (event.eventUsers.isNotEmpty) {
      final results = await GetNProfilesUseCase().run(GetNProfilesUseCase.input(event.eventUsers, 3));
      emit(RegionEventProfileMembersStateMapper().mapResultsToState(currentState: state, results: results));
    }
  }
}
